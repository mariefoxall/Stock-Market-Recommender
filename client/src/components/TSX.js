...
const TSX = () => {
  ...
  const showRecommendation = async () => {
    if (!tsxList.includes(stockSymbol.toUpperCase())) {
      ...
      <PageDiv>
        <InputSection>
          ...
          <InputDiv>
            ...
            {!validSymbol && (
              <NotFound>
                We can't find that symbol on the TSX! Please try again.
              </NotFound>
            )}
            <DateDiv>
